...
class SmartRecruitersApplier(BaseApplier):
    """
    Postula automáticamente en tableros de SmartRecruiters (careers.smartrecruiters.com).
    """
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"  [SMARTRECRUITERS] Error cargando cv_erick_data.json: {e}")

    async def apply(self, job: dict) -> ApplyResult:
        url = job.get('url', '')
        title = job.get('title', 'Sin Título')
        company = job.get('company', 'SmartRecruiters')

        if not url:
            return ApplyResult(title, company, "SmartRecruiters", url, False, "URL vacía")
        ...
        try:
            async with async_playwright() as p:
                ...
                print(f"  [SMARTRECRUITERS] Navegando a {url}")
                ...
                if await file_input.count() > 0:
                    if os.path.exists(self.cv_path):
                        print("  [SMARTRECRUITERS] Subiendo CV...")
                ...
                submit_btn = page.locator("button[type='submit'], button:has-text('Submit Application')")
                if await submit_btn.count() > 0:
                    await submit_btn.first.click()
                    await self._human_delay(2.0, 3.5)

                await context.close()
                return ApplyResult(title, company, "SmartRecruiters", url, True, "Postulación enviada exitosamente a SmartRecruiters")

        except Exception as e:
            return ApplyResult(title, company, "SmartRecruiters", url, False, f"Error SmartRecruiters: {str(e)[:100]}")