...
class BambooHRApplier(BaseApplier):
    """
    Postula automáticamente en tableros de BambooHR (bamboohr.com, careers.bamboohr.com).
    """
    ...
    def __init__(self):
        ...
        try:
            ...
            print(f"  [BAMBOOHR] Error cargando cv_erick_data.json: {e}")

    async def apply(self, job: dict) -> ApplyResult:
        url = job.get('url', '')
        title = job.get('title', 'Sin Título')
        company = job.get('company', 'BambooHR')

        if not url:
            return ApplyResult(title, company, "BambooHR", url, False, "URL vacía")
        ...
        try:
            async with async_playwright() as p:
                ...
                print(f"  [BAMBOOHR] Navegando a {url}")
                await page.goto(url, wait_until="domcontentloaded", timeout=45000)
                await self._human_delay(2.0, 4.0)

                # BambooHR a menudo tiene un botón de aplicación
                ...
                await context.close()
                return ApplyResult(title, company, "BambooHR", url, True, "Postulación enviada exitosamente a BambooHR")

        except Exception as e:
            return ApplyResult(title, company, "BambooHR", url, False, f"Error BambooHR: {str(e)[:100]}")